...
const GraphItem: React.FC<IGraphItem> = ({
     ...
     const topPosition =
          (graphHeight / 4) * statusPosition[status] - (1 + graphHeight / 8) ||
          0;
     ...
     const horizontalHeight =
          (graphHeight / 4) *
          Math.abs(statusPosition[lastStatus] - statusPosition[status]) ||
          0;
     ...
     const getStatusDuration = (start: any, end: any) => {
          const { days, hours, minutes, seconds } = getDurationDate(parseUnix(start), parseUnix(end));
          return (
     ...
     return (
          <React.Fragment>
               {status === "intermediate" ? (
                    ...
                    >
                         <span
                              ...
                              onMouseEnter={() =>
                                   !currentLog && setHoveredId?.(item._id)
                              }
               ...
               <div
                    className={`range-wrapper ${currentLog?._id === item._id ? "active" : ""
                         }`}
               >
                    <div
