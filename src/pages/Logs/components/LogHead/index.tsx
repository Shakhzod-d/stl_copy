...
const LogsHead: React.FC<ILogsHead> = ({
     ...
     const getStatusTotalTime = (statusType: TItemStatus) => {
          const totalSum = () =>
               logs.reduce(
                    (prev, log) =>
                         log.status === statusType ? log.duration + prev : prev,
                    initialTime || 0
               );
          return {
          ...
          <div className="logs-inner-head">
               <div className="driver-info">
                    ...
                    <p className="driver-info-desc">
                         Phone No: {driverData.phone}
                    </p>
                    <p className="driver-info-desc">
                         {/* Driver ID: {driverData.driverId} */}
                    </p>
                    <p className="driver-info-desc">
                         Mongo ID: {driverData._id}
                    </p>
               </div>
               <div className="driver-progress">
                    ...
                    <div className="driver-progress-bar">
                         ...
                         <CircleProgress
                              value={
                                   (cycle.drive / DRIVE_TIME_LIMIT) * 100
                              }
                              title="drive"
                              color="#5CB176"
                              // color={
                         ...
                         <CircleProgress
                              value={
                                   (cycle.shift / SHIFT_TIME_LIMIT) * 100
                              }
                              title="shift"
                              color="#66B7F1"
                              time={
                         ...
                         <CircleProgress
                              value={
                                   (cycle.cycle / CYCLE_TIME_LIMIT) * 100
                              }
                              title="cycle"
                              color="#EE5E52"
                              time={
...
const CircleProgress: React.FC<Props> = ({ title, value, time, color }) => {
     return (
          <Progress
               type="circle"
               percent={value}
               ...
               format={() => (
                    <div className="logs-progress-content">
                         <div
                              className="circle"
                              style={{
                                   transform: `translate(-50%, -50%) rotate(${3.6 * value - 6
                                        }deg)`,
                              }}
                         />
