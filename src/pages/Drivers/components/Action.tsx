...
import { EMAIL_PATTERN, formProps, issue_stats, select_paging } from "@/constants";
...
const Action = () => {
  ...
  // Get data
  const { data: drivers, isLoading: driverLoad } = useApi<{ data: IDriverData[] }>("/drivers", select_paging, { suspense: true });
  const { data: vehicles, isLoading: vehicleLoad } = useApi<{ data: IVehicleData[]; total: number }>("/vehicles", select_paging, { suspense: true });
  const { data } = useApi<IDriverData>(`/driver/${id}`, {}, { enabled: Boolean(id), suspense: true })

  // Action mutation
  const { mutate: createMutation, isLoading: createLoading } = useApiMutation("/driver");
  const { mutate: updateMutation, isLoading: updateLoading } = useApiMutationID("PUT", "/driver");
