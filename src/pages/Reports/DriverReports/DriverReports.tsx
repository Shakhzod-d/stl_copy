...
const DriverReports = () => {
     ...
     const { data: DriverReports, isFetching } = useApi<IDriverReport[]>(
          `driver/report`,
          queryParams,
          {
               enabled:
                    ...
                    const driverReportPdfs: IDriverReportPDF[] = data.data.map(
                         (driverReport) => {
                              if (driverReport?.log) {
                                   ...
                                   return {
                                        log:
                                             mapDriverLogs(driverReport.log) ||
                                             [],
     ...
     const getReportFileName = () => {
          return (
               "Driver Reports from" +
               moment
                    .unix(queryParams?.from || moment().unix())
                    .format("YYYY-MM-DD") +
               "to" +
               moment
                    .unix(queryParams?.to || moment().unix())
                    .format("YYYY-MM-DD")
          );
     };
               ...
               <div className="drivers-reports-header">
                    ...
                    <div className="form">
                         <Select
                              ...
                              setValue={(val) => setSelectedDriver(val)}
