...
const Trackings: React.FC = () => {
     ...
     const [time, setTime] = useQueryParam("time", withDefault(NumberParam, momentZone().valueOf()))
                    ...
                    <div className="logs-body mt-24">
                         <Table
                              ...
                              onRow={({ _id }: IDriverData) => {
                                   return {
                                        onClick: () => historyPush(`/main/trackings/inner/${_id}?time=${momentZone(time).unix()}`)
                                   };
                              }}
                         />
                    </div>
