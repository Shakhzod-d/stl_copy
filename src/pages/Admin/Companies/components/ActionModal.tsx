...
const ActionModal: React.FC<Props> = ({ toggle, id, onSuccess }) => {
     ...
     const { data, isLoading } = useApi(`/company/${id}`, {}, { enabled: Boolean(id) });

     // get Services
     const { data: serviceData, isLoading: servicesLoad } = useApi("/main", select_paging);

     //action mutations
     const { mutate: createMutate, isLoading: createLoading } = useApiMutation("/company");
     const { mutate: updateMutate, isLoading: updateLoading } = useApiMutationID("PUT", "/company");
