...
const ActionModal: React.FC<Props> = ({ toggle, id, onSuccess }) => {
     ...
     const { data, isLoading } = useApi(`/onlyCompany/${id}`, {}, { enabled: Boolean(id) });

     //action mutations
     const { mutate: createMutate, status: createStatus } = useApiMutation("/onlyCompany");
     const { mutate: updateMutate, isLoading: updateLoading } = useApiMutationID("PUT", "/onlyCompany");
