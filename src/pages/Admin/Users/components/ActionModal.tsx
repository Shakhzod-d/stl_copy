...
import { EMAIL_PATTERN, formProps, role_names, select_paging } from "@/constants";
...
const ActionModal: React.FC<Props> = ({ toggle, id, onSuccess }) => {
     ...
     const { handleSubmit, control, reset, watch, getValues } = useForm<IUserForm>(formProps);
     ...
     // get Services
     const { data: servicesData, isLoading: servicesLoad } = useApi("/main", select_paging);
     const { data: companiesData, isLoading: companiesLoad } = useApi(`/main/${getValues("serviceId")}`, select_paging, { enabled: Boolean(watch("serviceId")) });
